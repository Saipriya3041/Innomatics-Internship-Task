...
def compare_products(product_id_1: int = Query(...), product_id_2: int = Query(...)):
    ...
    return {'product_1': p1, 'product_2': p2, 'better_value': cheaper['name'], 'price_diff': abs(p1['price'] - p2['price'])}
...
def search_products(keyword: str = Query(...)):
    ...
    return {'matched_products': result, 'total_found': len(result)}
...
def sort_products(sort_by: str = Query('price'), order: str = Query('asc')):
    ...
    return {'sorted_products': sorted_list, 'sort_by': sort_by, 'order': order}

@app.get('/products/page')
def paginate_products(page: int = Query(1), limit: int = Query(2)):
    if page < 1 or limit < 1:
        return {'error': 'page and limit must be >= 1'}
    start = (page - 1) * limit
    end = start + limit
    paginated = products[start:end]
    total_pages = -(-len(products) // limit)
    return {'page': page, 'limit': limit, 'total_products': len(products), 'total_pages': total_pages, 'products': paginated}

@app.post('/products')
def add_product(new_product: NewProduct, response: Response):
    next_id = max(p['id'] for p in products) + 1
    product = {'id': next_id, 'name': new_product.name, 'price': new_product.price,
               'category': new_product.category, 'in_stock': new_product.in_stock}
...
def browse_products(
    keyword: str = Query(None, description="Search keyword in product name"),
    sort_by: str = Query('price', description="Sort by 'price' or 'name'"),
    order: str = Query('asc', description="'asc' or 'desc'"),
    page: int = Query(1, ge=1, description="Page number, >=1"),
    limit: int = Query(4, ge=1, le=20, description="Items per page, 1-20")
):
    # Step 1: Search
    result = products
    if keyword:
...
def place_order(order_data: OrderRequest):
    ...
    order = {'order_id': order_counter, 'customer_name': order_data.customer_name,
             ...
             'delivery_address': order_data.delivery_address, 'total_price': total,
             'status': 'confirmed'}
    orders.append(order)
    order_counter += 1
...
def get_orders_paged(
    page: int = Query(1, ge=1, description="Page number, >=1"),
    limit: int = Query(3, ge=1, le=20, description="Number of orders per page")
):
...
def add_to_cart(product_id: int = Query(...), quantity: int = Query(1)):
    ...
    cart_item = {'product_id': product_id, 'product_name': product['name'], 'quantity': quantity,
                 'unit_price': product['price'], 'subtotal': calculate_total(product, quantity)}
...
def checkout(checkout_data: CheckoutRequest, response: Response):
    ...
    grand_total = 0
    for item in cart:
        order = {'order_id': order_counter, 'customer_name': checkout_data.customer_name,
                 'product': item['product_name'], 'quantity': item['quantity'],
                 'delivery_address': checkout_data.delivery_address, 'total_price': item['subtotal'],
                 'status': 'confirmed'}
        orders.append(order)
